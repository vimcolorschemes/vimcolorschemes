...
test("getRepositoryInfos adding valid date field", () => {
  expect(typeof getRepositoryInfos({ createdAt: new Date() }).createdAt).toBe(
    "string",
  );
  expect(
    typeof getRepositoryInfos({ lastCommitAt: new Date() }).lastCommitAt,
  ).toBe("string");
});

test("getRepositoryInfos adding valid stringified date field", () => {
  expect(typeof getRepositoryInfos({ createdAt: "2020-03-03" }).createdAt).toBe(
    "string",
  );
  expect(
    typeof getRepositoryInfos({ lastCommitAt: "2020-03-03" }).lastCommitAt,
  ).toBe("string");
});

test("getRepositoryInfos adding invalid stringified date field", () => {
  expect(getRepositoryInfos({ createdAt: "abcd" }).createdAt).toBe(
    "Invalid date",
  );
  expect(getRepositoryInfos({ lastCommitAt: "reobin-dev" }).lastCommitAt).toBe(
    "Invalid date",
  );
});
